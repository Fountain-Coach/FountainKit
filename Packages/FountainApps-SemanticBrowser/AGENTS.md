...
- Control bridge status: JSCore midi2 bridge scaffold exists (`Midi2JSBridge`); extend to load full midi2.js bundle (env `SB_MIDI2_BUNDLE`), expose `/midi2/status` for health/capability probing, and wire `scheduleUMP` + capability feed to the stage. Planner should read `/webgpu/capabilities` and `/midi2/status` when selecting plans.
