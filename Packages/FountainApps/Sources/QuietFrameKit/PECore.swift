...
@MainActor public final class QFPEClientCore {
    ...
    // Nonisolated entry: feed UMP words (SysEx7) and decode; all delegate
    // callbacks are marshalled back to the main actor.
    nonisolated public func handleSysEx7UMP(words: [UInt32]) {
        Task { @MainActor [weak self] in
            ...
            if let data = try? JSONSerialization.data(
                withJSONObject: ["dir": "in", "ump": words.map { String(format: "0x%08X", $0) }],
                options: []
            ), let s = String(data: data, encoding: .utf8) {
                self.delegate?.peDidEmitUMPEvent(json: s)
            }
        }
    }
}
