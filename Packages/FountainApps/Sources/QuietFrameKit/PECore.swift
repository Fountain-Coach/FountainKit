...
public final class QFPEClientCore {
    ...
    public nonisolated func handleSysEx7UMP(words: [UInt32]) {
                ...
                if pe.command == .getReply || pe.command == .notify {
                    if let obj = try? JSONSerialization.jsonObject(with: Data(pe.data)) as? [String: Any] {
                        if let data = try? JSONSerialization.data(withJSONObject: obj, options: [.prettyPrinted]), let s = String(data: data, encoding: .utf8) {
                            Task { @MainActor in self.delegate?.peDidUpdateSnapshot(json: s) }
                        }
                    }
                }
            default: break
        ...
        if let data = try? JSONSerialization.data(withJSONObject: ["dir":"in","ump": words.map { String(format: "0x%08X", $0) }], options: []), let s = String(data: data, encoding: .utf8) {
            Task { @MainActor in self.delegate?.peDidEmitUMPEvent(json: s) }
        }
    }
}
