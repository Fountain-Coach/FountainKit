    ...
    init() {
        ...
        node = AVAudioSourceNode { [weak self] _, _, frameCount, audioBufferList -> OSStatus in
            ...
            for frame in 0..<n {
                var sample: Double = 0
                self.lock.lock()
                for (k, v) in self.active { var entry = v; sample += sin(entry.phase) * entry.amp; entry.phase += 2.0 * .pi * entry.freq / self.sampleRate; self.active[k] = entry }
                self.lock.unlock()
...
// startSynthIfNeeded implemented in Runner (main.swift)
