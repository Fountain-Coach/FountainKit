...
Task {
    ...
    // Register generated OpenAPI handlers via NIO bridge with fallback that serves the spec
    let fallback = HTTPKernel { req in
        if req.path == "/openapi.yaml" {
            let url = URL(fileURLWithPath: "Packages/FountainServiceKit-Planner/Sources/PlannerService/openapi.yaml")
