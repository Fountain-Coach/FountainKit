...
import SwiftUI
import AppKit
@testable import blank_page_app

@MainActor
final class BlankSnapshotTests: XCTestCase {
    ...
    func testSnapshots_blankPage() throws {
        for s in sizes {
            autoreleasepool {
                let hosting = NSHostingView(rootView: BlankRootView())
                ...
                let baselineURL = baselineDir().appendingPathComponent("\(s.name)/blank.png")
                ...
                if update {
                    ...
                    if let diff = SnapshotUtils.diffRMSE(img, baseline) {
                        if diff.rmse > 0.01 {
                            let artifacts = artifactsDir().appendingPathComponent("blank/\(Int(Date().timeIntervalSince1970))/\(s.name)")
    ...
    private func baselineDir() -> URL {
        URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
            .appendingPathComponent("Packages/FountainApps/Tests/BlankAppUITests/Baselines", isDirectory: true)
    }
