...
function parseSnapshot(events) {
  // Find last PE JSON snapshot
  for (let i = events.length - 1; i >= 0; i--) {
    const e = events[i]
    if (e.peJSON) {
      try {
        const obj = JSON.parse(e.peJSON)
        const props = (obj.properties || []).reduce((acc, it) => { acc[it.name] = it.value; return acc }, {})
        return { ts: e.ts, props }
      } catch {}
    }
  }
  return null
...
async function waitForSnapshot(prevTs = 0, tries = 10) {
  for (let i = 0; i < tries; i++) {
    const ev = await tail()
    const snap = parseSnapshot(ev)
    if (snap && snap.ts > prevTs) return snap
    await sleep(100)
  }
...
async function main() {
  ...
  let z = Number(snap.props['zoom'] ?? 1)
  let tx = Number(snap.props['translation.x'] ?? 0)
  let ty = Number(snap.props['translation.y'] ?? 0)

  // Pan invariants
  await mrts.panByView(PAN_DX, PAN_DY, TARGET_NAME)
  snap = await waitForSnapshot(snap.ts)
  const z2 = Number(snap.props['zoom'] ?? z)
  const tx2 = Number(snap.props['translation.x'] ?? tx)
  const ty2 = Number(snap.props['translation.y'] ?? ty)
  const expTx = tx + (PAN_DX / z)
  const expTy = ty + (PAN_DY / z)
  ...
  // Anchor-stable zoom invariant
  const anchorDoc = viewToDoc(AX, AY, z2, tx2, ty2)
  await mrts.zoomAround(AX, AY, MAG, TARGET_NAME)
  snap = await waitForSnapshot(snap.ts)
  const z3 = Number(snap.props['zoom'] ?? z2)
  const tx3 = Number(snap.props['translation.x'] ?? tx2)
  const ty3 = Number(snap.props['translation.y'] ?? ty2)
