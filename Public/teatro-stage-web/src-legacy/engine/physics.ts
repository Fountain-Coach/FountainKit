...
// Thin wrappers around Cannon‑ES to keep the existing API shape used by the rig and tests,
// while delegating integration and contact behaviour to the professional engine.
...
export class DistanceConstraint implements Constraint {
  ...
  constructor(bodyA: Body, bodyB: Body, restLength: number, stiffness = 1.0) {
    // Cannon's DistanceConstraint takes stiffness as a "max force" parameter; use a scaled value.
...
export class GroundConstraint implements Constraint {
  ...
  attach(world: World): void {
    // Install a static ground plane once per world; Cannon handles collision.
    ...
    // No per‑body constraint necessary; the foot body already has a box collider and will
    // collide with the static plane installed above.
    void this.body;
  }
}
