...
- mvk-runtime-smoke — `bash Scripts/ci/mvk-runtime-smoke.sh` builds and runs the lightweight `mvk-runtime-tests` executable to exercise the MetalViewKit runtime locally. It verifies: health (`GET /health`) and event echo (`POST /v1/midi/events` → `GET /v1/midi/vendor`). Writes JSON summary to `.fountain/logs/mvk-runtime-smoke-*.json` and exits non‑zero on failure.
- mvk-runtime-probe — `bash Scripts/ci/mvk-runtime-probe.sh` boots the server and probes key endpoints without the test runner. It checks health/clock, injects a UMP batch, and asserts echo via `GET /v1/midi/vendor`.
